                                           ...
                                           LocalSearchParams& lsps) const
{
    ...
    auto gnbDev = gnbDevice->GetObject<NrGnbNetDevice>();
    auto phy = gnbDev->GetPhy(m_primaryCarrierIndex);
    auto spectrumPhy = phy->GetSpectrumPhy();
    ...
    auto rowElemsPerPort = antenna.gnbArrayModel->GetVElemsPerPort();
    auto colElemsPerPort = antenna.gnbArrayModel->GetHElemsPerPort();
...
NrInitialAssociation::ComputeRxEnergy(Ptr<const SpectrumSignalParameters> spectrumSigParam) const
{
    ...
    for (size_t iRb = m_startSsb; iRb < m_startSsb + m_numBandsSsb; iRb++)
    {
        ...
        {
            energyPerRb += norm(spectrumChannelMatrix->operator()(idxAnt, 0, iRb));
        }
        engergyTotal += energyPerRb;
    }
    return engergyTotal;
}
